...
本模块是系统的效果评估与监测舱，负责发起 AI 可见度诊断任务，采集各大语言模型（LLM）的真实回答，并对品牌提及、来源引用、情感极性及流量报表进行分析与可视化展示。
...
### 1.2 检索截图库 (快照追踪)
* **用户路径**：用户在诊断结果中点击“查看快照”，或者在“检索截图库”中根据日期、关键词、品牌筛选历史截图。
* **业务产出**：直观展示发起诊断当时，大模型回答结果的完整网页长截图（带时间戳水印），作为品牌上榜和被提及的真实司法级凭证，供结案汇报使用。
